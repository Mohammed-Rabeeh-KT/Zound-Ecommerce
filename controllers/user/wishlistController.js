...
const addToWishlist = catchAsync(async (req, res, next) => {
    ...
    if (!wishlist) {
        ...
        const existingItem = wishlist.products.find(item => {
            ...
            // If no variant specified, check for any matching product
            if (!finalVariantId && !item.variantId) return true;

            // Match by variant
            return item.variantId && item.variantId.toString() === finalVariantId?.toString();
        });
