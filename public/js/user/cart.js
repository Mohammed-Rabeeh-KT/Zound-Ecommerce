...
async function removeFromCart(productId, variantId) {
    ...
    if (result.isConfirmed) {
        try {
            const url = variantId
                ? `/api/user/cart/remove/${productId}?variantId=${variantId}`
...
async function applyDiscount() {
    ...
    try {
        const response = await axios.post('/api/user/checkout/apply-coupon', { code });
...
async function proceedToCheckout() {
    ...
    try {
        ...
        const response = await axios.get('/api/user/cart/validate-stock');
